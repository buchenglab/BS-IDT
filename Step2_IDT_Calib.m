...
[ freqXY, con, radP, xI, yI, uI, vI, XYmid ] = calCoord(freqUV,imSz, Pixelsize * Mag, Mag, NA, lambda );
...
[FIdiv, FIdivG, FI, w_2NA] = calFI(I_Calib, xI, yI, XYmid, radP, sigmaG);
...
[ freqDTh3, rad_cal] = calCircEdge(FIdivG(:,:,~DFI), I_Calib(:,:,~DFI), radP, freqDTh(~DFI,:), XYmid, xI, yI, sigmaG, rScan, thScan, dScan, calRad, con, lambda);
...
if removeOut
    ...
    %When including DFI, it doesn't use the darkfield to fit but it does
...
[Ini_NAy_fit, Ini_NAx_fit] = nonlinearFit(Ini_NAx_Cal, Ini_NAy_Cal, Bright_Radius, fDL, Sorted_Pos, lambda);
